...
% While these minimizers may be unique, the set $\mcl{M}(\mu)$ is still not convex, hence multiple local minimizers may be characterized as in~\cite[Theorem~3.2]{Gugercin2008}.
...
\begin{algorithm}\label{alg:irka}
    ...
    \KwData{Initial interpolation points ${\sset{\sigma_i^{(0)}}}_{i = 1}^r$ closed and initial tangent directions ${\sset{r_i^{(0)}}}_{i = 1}^r, {\sset{\ell_i^{(0)}}}_{i = 1}^r$ under complex conjugation, full order model $\Sigma$}
    ...
    \While{not converged}{
        ...
        $\zeta_r(s) = \tilde{C} {(s \tilde{E} - \tilde{A})}\inv \tilde{B} = \sum\limits_{i = 1}^r \frac{\tilde{\ell}_i \tilde{r}_i\trans}{s - {\lambda(\sigma^{(k)})}_i}$\;

...
We first give a description of \ac{LTI} \ac{PH} \acp{DAE} before mentioning more generalized versions.
...
We could now rephrase~\eqref{eq:lti} in terms of these matrices, however there is one more aspect to the \ac{PH} formulation we yet lack: the \emph{Hamiltonian}.
...
\begin{definition}\label{def:phlti}
    A \acf{PH} \ac{LTI} system with quadratic Hamiltonian is a system of equations
...
\begin{remark}
    Because we inherently derived this representation from the matrices $(A, B, C, D, E)$, we can in turn describe any \ac{LTI} \ac{PH} system $\Sigma_{\msc{phlti}}$ as an \ac{LTI} system $\Sigma_{\msc{lti}}$ by defining its system matrices to be
    \begin{equation*}
...
Additionally, while we motivated the dissipation inequality as a property of \ac{PH} systems, it is by no means restricted to \ac{PH} systems, in and of itself only requiring the existence of an energy storage function $\mcl{H}$ to serve as a proxy for the total system energy.
...
\begin{remark}
    Any passive \ac{LTI} system $\Sigma_{\msc{lti}}$ given by a realization $(A, B, C, D, E)$ with a quadratic Hamiltonian $\mcl{H}(x) = x\trans H x$ allows for a \ac{PH} $\Sigma_{\msc{phlti}}$ if the matrix $H$ is positive semidefinite and it satisfies $\knl{H} \subseteq \knl{A}$; cf.~\cite[Theorem~7.1]{VanDerSchaft2014}.
\end{remark}
...
\begin{remark}
    ...
    The \ac{PH} constraints on the dissipative elements $P, R$, and $S$ stays the same as for a time-invariant system $\Sigma_{\msc{phlti}}$, however the condition for the previously simply skew-symmetric part shifts to the need for a skew-adjoint differential operator; cf.~\cite[Definition~4.8]{Mehrmann2022}.
\end{remark}
...
\begin{remark}
    ...
    This then results in a nonlinear \ac{PH} system such as
    ...
    \end{equation*}
    alongside similar constraints on the energy-conservative and dissipative parts of the \ac{PH} system as in the \ac{LTI} case; cf.~\cite[Definition~4.1]{Mehrmann2022}.
\end{remark}

\begin{remark}
    For the most general representation of \ac{PH} systems we have to take a large conceptual leap, for a more detailed discussion we refer to~\cite[Section~2.2]{VanDerSchaft2014}.
    ...
    \end{enumerate}
    In this setting we can thus define a \ac{PH} system to be the Dirac structure
    \begin{equation*}
